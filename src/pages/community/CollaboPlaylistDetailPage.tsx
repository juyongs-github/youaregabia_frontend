...
import { FaCalendarAlt, FaClock, FaComment, FaCrown, FaEdit, FaExclamationTriangle, FaHeart, FaInfoCircle, FaMusic, FaPause, FaPlay, FaPlus, FaRedo, FaSearch, FaStopwatch, FaThumbsUp, FaTrash, FaUser, FaUsers } from "react-icons/fa";
...
function CollaboPlaylistDetailPage() {
  ...
  useEffect(() => {
    if (!playlist?.deadline) { setTimeLeft(""); return; }
  ...
  const fetchPlaylist = async () => {
    ...
    try {
      const res = await playlistApi.getCollaborativePlaylist(Number(id), user?.email);
  ...
  const handleSuggestSong = async (song: Song, reason: string) => {
    if (!id || !user?.email) { alert("로그인이 필요합니다."); return; }
  ...
  const handlePlayAll = () => {
    ...
    setPlayerSongs(songs); setPlayerIndex(0); setIsPlayerVisible(true);
  };
  const handlePlaySong = (index: number) => {
    setPlayerSongs(songs); setPlayerIndex(index); setIsPlayerVisible(true);
  };

  const handleReopen = async () => {
    if (!id || !user?.email) { alert("로그인이 필요합니다."); return; }
    if (!reopenDeadlineDate) { alert("새 마감일을 설정해주세요."); return; }
  ...
  useEffect(() => { fetchPlaylist(); fetchSongs(); }, [id, user?.email]);
  ...
  return (
    <div className="flex flex-col w-full gap-12">
      <button className="flex items-center gap-3 self-start" onClick={() => navigate("/community/collabo")}>
        <GrFormPrevious size={30} /><span className="text-xl font-bold">목록</span>
      </button>

      {/* 플레이리스트 정보 */}
      <div className="flex gap-10">
        <div className="flex-shrink-0 w-64 h-64 bg-slate-700 rounded-2xl overflow-hidden">
          {playlist?.imageUrl
        ...
        <div className="flex flex-col justify-between flex-1 min-w-0 py-1">
          <div className="flex flex-col gap-2">
            ...
            <div className="flex items-center gap-3">
              ...
              {isClosed
                ? <span className="px-3 py-1 rounded-full text-sm font-semibold bg-gray-500/20 text-gray-400 border border-gray-500/30">마감</span>
                : <span className="px-3 py-1 rounded-full text-sm font-semibold bg-green-500/20 text-green-300 border border-green-500/30">참여 진행중</span>
              }
            </div>
            {playlist?.description && <p className="text-gray-300 line-clamp-3">{playlist.description}</p>}
          </div>
          <div className="flex flex-wrap items-center gap-5 text-base text-gray-400 mt-4">
            {playlist?.participantCount !== undefined && (
              <div className="flex items-center gap-1.5"><FaUsers size={16} /><span>{playlist.participantCount}명 참여</span></div>
            )}
            <div className="flex items-center gap-1.5"><FaMusic size={16} /><span>{songs.length}곡</span></div>
            {playlist?.createdAt && <div className="flex items-center gap-1.5"><FaClock size={16} /><span>{timeAgo(playlist.createdAt)}</span></div>}
            {playlist?.creatorEmail && <div className="flex items-center gap-1.5"><FaCrown size={16} className="text-yellow-400" /><span>등록자: </span><span className="text-white">{playlist.creatorEmail}</span></div>}
          </div>
          {playlist?.deadline && (
          ...
          <div className="flex items-center gap-3 mt-2">
            {songs.length > 0 && (
              <button onClick={handlePlayAll} className="flex items-center gap-2 px-5 py-2.5 bg-red-600 hover:bg-red-700 transition-colors rounded-full text-base font-semibold">
            ...
            {isCreator && (
              <>
                {!isClosed && (
                  <button onClick={() => setShowEditModal(true)} className="flex items-center gap-2 px-4 py-2.5 bg-white/10 hover:bg-white/20 transition-colors rounded-full text-base font-semibold">
                    <FaEdit size={13} />수정
                  </button>
                )}
                {isClosed && (
                  <button onClick={() => setShowReopenPanel(v => !v)} className="flex items-center gap-2 px-4 py-2.5 bg-green-600/20 hover:bg-green-600/40 text-green-400 transition-colors rounded-full text-base font-semibold">
                    <FaRedo size={13} />참여 재개
                  </button>
                )}
                {!isClosed && playlist?.deadline && (
                  <button onClick={handleEarlyClose} className="flex items-center gap-2 px-4 py-2.5 bg-orange-600/20 hover:bg-orange-600/40 text-orange-400 transition-colors rounded-full text-base font-semibold">
                    <FaClock size={13} />마감하기
                  </button>
                )}
                <button onClick={handleDeletePlaylist} className="flex items-center gap-2 px-4 py-2.5 bg-red-600/20 hover:bg-red-600/40 text-red-400 transition-colors rounded-full text-base font-semibold">
                  <FaTrash size={13} />삭제
                </button>
              </>
            )}
      ...
      {isCreator && isClosed && showReopenPanel && (() => {
        ...
        return (
          <div className="p-5 rounded-xl bg-green-900/20 border border-green-500/30 flex flex-col gap-3">
            <p className="text-base font-semibold text-green-300">새 마감일 설정</p>
              ...
              <DatePicker
                ...
                customInput={
                  <div style={{
                    ...
                    padding: "14px", borderRadius: "12px", background: "#1b2b3b",
                    border: "1.5px solid transparent",
                    color: reopenDeadlineDate ? "white" : "#6b8099",
                    fontSize: "15px", cursor: "pointer", userSelect: "none", width: "100%",
                  }}>
                    <span>{reopenDeadlineDate
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                      {reopenDeadlineDate && (
                        <span onClick={(e) => { e.stopPropagation(); setReopenDeadlineDate(null); }}
                          style={{ color: "#6b8099", fontSize: "18px", lineHeight: 1, cursor: "pointer" }}>×</span>
                      )}
            ...
            <div className="flex gap-2">
              <button onClick={handleReopen} className="flex-1 py-2.5 bg-green-600 hover:bg-green-700 text-white text-base font-semibold rounded-lg transition-colors">
                확인
              </button>
              <button onClick={() => setShowReopenPanel(false)} className="flex-1 py-2.5 bg-white/10 hover:bg-white/20 text-gray-300 text-base rounded-lg transition-colors">
                취소
              </button>
            </div>
          </div>
      ...
      {user && (
        <div className="text-base text-gray-400 bg-yellow-500/30 border border-white/10 rounded-lg px-5 py-4 flex flex-col gap-1.5">
          <p className="font-semibold text-yellow-300 flex items-center gap-2"><FaExclamationTriangle size={13} />주의사항</p>
          <p className="text-sm">• 1인당 최대 5곡까지 추가할 수 있으며 최대 3곡까지 투표할 수 있습니다.</p>
          <p className="text-sm">• 등록자의 설정한 마감 시간까지 곡 추가와 투표를 진행할 수 있습니다.</p>
          <p className="text-sm">• 등록자의 요청에 따라 조기 마감 될 수 있으며, 참여자가 추가한 수록곡이 삭제 될 수 있습니다.</p>
          <p className="text-sm">• 마감 시간이 지나면 곡 추가와 투표가 불가능하며, 투표 순위가 가장 높은 상위 10곡이 최종 수록곡으로 확정됩니다.</p>
          <p className="text-sm">• 마감 시간까지 추가된 곡이 3곡 미만일 경우 해당 플레이리스트는 자동 삭제됩니다.</p>
        </div>
      )}
      ...
      {isPlaylistLoading ? (
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-5 mb-3">
            <h2 className="text-2xl font-bold shrink-0">수록곡</h2>
            {!isClosed && songs.length > 0 && (
              <div className="relative flex-1">
                <FaSearch size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" />
                <input
                  value={songFilter}
            ...
            : songs.length === 0
              ? (
                <div className="flex flex-col items-center gap-3 py-16 text-gray-400">
                  ...
                  <p>아직 수록곡이 없습니다.</p>
                  {!isClosed && <p className="text-sm">오른쪽에서 곡을 검색하여 추가해보세요.</p>}
                </div>
              ) : (() => {
                const sorted = songs.slice().sort((a, b) => (b.voteCount ?? 0) - (a.voteCount ?? 0));
                const displaySongs = isClosed
                  ? sorted.slice(0, 10)
                  : sorted.filter(s =>
                      !songFilter.trim() ||
                    ...
                    {displaySongs.map((song, index) => {
                      ...
                      return (
                        <div
                          ...
                          className={"flex items-center gap-5 p-5 rounded-xl transition-colors group cursor-pointer bg-gray-900 hover:bg-gray-800"}
                        >
                          {/* 순위 번호 (마감 후) */}
                          {isClosed && (
                          ...
                          {/* 앨범 아트 */}
                          <div className="relative w-24 h-24 shrink-0 rounded-xl overflow-hidden bg-slate-600">
                            {song.imgUrl
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 min-w-0">
                              <p className="text-lg font-semibold truncate" title={song.trackName}>{song.trackName}</p>
                              {song.suggestedByEmail === user?.email && (
                                <span className="shrink-0 px-2 py-0.5 rounded-full text-xs font-semibold bg-blue-500/20 text-blue-300 border border-blue-500/30">내가 추가한 곡</span>
                              )}
                            </div>
                            <p className="text-base text-gray-400 truncate flex items-center gap-1 mt-0.5">
                              {song.artistName}
                              {song.genreName && <><GoDotFill size={8} className="shrink-0" />{song.genreName}</>}
                            </p>
                            {song.suggestedByEmail && (
                              <p className="flex items-center gap-1 mt-1">
                                ...
                                <span className="text-sm text-white truncate">{song.suggestedByEmail}</span>
                              </p>
                            )}
                            {song.reason && (
                              <p className="flex items-start gap-1 mt-1.5">
                                <FaComment size={15} className="text-gray-500 shrink-0 mt-0.5" />
                                <span className="text-sm text-gray-400">추가 이유: </span>
                                <span className="text-sm text-white w-3/4">{song.reason}</span>
                              </p>
                            )}
                          </div>
                          ...
                          {!isClosed && canDelete(song) && (
                            <button
                              ...
                              className="flex items-center justify-center w-8 h-8 rounded-full bg-red-500/20 text-red-400 hover:bg-red-500/40 hover:text-red-300 transition-colors opacity-0 group-hover:opacity-100 shrink-0"
                            >
                              <FaTrash size={13} />
                            </button>
                          )}
        ...
        {!isClosed && (
          <div className="w-96 shrink-0">
            <h2 className="mb-4 text-2xl font-bold">곡 추가</h2>
            <div className="flex gap-2 mb-4">
              ...
              />
              <button onClick={handleSearch} className="px-4 py-2 bg-red-600 rounded-lg hover:bg-red-700 transition-colors">
                <FaSearch size={16} />
              </button>
            </div>
            {isSearching
                ...
                : (
                  <div className="space-y-2 max-h-[520px] overflow-y-auto pr-1">
                    {searchResults.map(song => (
                      <div key={song.id} className="flex items-center gap-4 p-4 rounded-xl bg-white/5 hover:bg-white/10 transition-colors">
                        <div className="relative w-16 h-16 shrink-0 rounded-xl overflow-hidden bg-slate-600">
                          {song.imgUrl
                            ? <img src={song.imgUrl} alt="" className="w-full h-full object-cover" />
                            : <div className="flex items-center justify-center w-full h-full"><FaMusic size={18} className="text-white opacity-40" /></div>
                          }
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-base font-semibold truncate" title={song.trackName}>{song.trackName}</p>
                        ...
                        {song.previewUrl && (
                          ...
                          >
                            {isPlayerVisible && playerSongs[0]?.id === song.id && !isPreviewPaused ? <FaPause size={13} /> : <FaPlay size={13} />}
                          </button>
                        )}
                        <button
                          onClick={() => { setSuggestTarget(song); setSuggestReason(""); }}
                          disabled={!user}
                          className="flex items-center justify-center w-9 h-9 rounded-full bg-red-600 hover:bg-red-700 transition-colors shrink-0 disabled:opacity-40 disabled:cursor-not-allowed"
                        >
                          <FaPlus size={15} />
                        </button>
                      </div>
            ...
            {!user && hasSearched && (
              <p className="text-center text-sm text-gray-400 mt-2">로그인 후 곡을 추가할 수 있습니다.</p>
            )}
          </div>
        )}
      </div>}
          ...
          <div className="bg-[#1a2535] rounded-2xl p-6 w-full max-w-md mx-4 flex flex-col gap-7">
            ...
            <div className="flex items-center gap-3 bg-white/5 rounded-xl p-3">
              {suggestTarget.imgUrl && (
                <img src={suggestTarget.imgUrl} className="w-10 h-10 rounded-lg object-cover shrink-0" />
              )}
              <div className="flex flex-col min-w-0">
                <span className="text-white font-bold truncate">{suggestTarget.trackName}</span>
                <p className="text-sm text-gray-400 truncate flex items-center gap-1 mt-0.5">
                  {suggestTarget.artistName}
                  {suggestTarget.genreName && <><GoDotFill size={8} className="shrink-0" />{suggestTarget.genreName}</>}
                </p>
              </div>
            </div>
            <div className="flex flex-col gap-2">
              <div className="flex justify-between items-center p-1">
                ...
                <span className="text-sm" style={{ color: suggestReason.length > 50 ? "#f87171" : "#6b8099" }}>
