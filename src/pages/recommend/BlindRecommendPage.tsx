...
const BlindRecommendPage = () => {
  ...
  const loadInitialSongs = useCallback(
    async (mode: TotalMode) => {
      ...
      if (initialSongs.length > 0) {
        ...
        setCurrentIndex(0); // 인덱스 초기화 보장
        setPhase("playing"); // 데이터 로드 후 phase 변경
      } else {
  ...
  const handleStart = (mode: TotalMode) => {
    setTotalMode(mode);
    setLikedSongs([]);
    loadInitialSongs(mode);
  };

  // 플레이리스트 데이터 페칭 로직 (기존 동일)
  ...
  // 모드 변경 처리
  const handleModeChange = (mode: TotalMode) => {
    if (window.confirm("모드를 변경하면 진행 상황이 초기화됩니다. 계속할까요?")) {
      setTotalMode(mode);
      setCurrentIndex(0);
      setLikedSongs([]);
      loadInitialSongs(mode);
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-12 text-white flex flex-col md:flex-row gap-8 items-start relative min-h-[80vh]">
      {/* 좌측 상단 모드 설정 */}
      <div className="absolute top-0 left-4 flex bg-neutral-900/80 p-1 rounded-xl border border-neutral-800 backdrop-blur-sm shadow-xl">
        {(["infinite", 5, 10] as const).map((mode) => (
          <button
            key={mode}
            onClick={() => handleModeChange(mode)}
            className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${
              totalMode === mode ? "bg-indigo-600 text-white" : "text-gray-500 hover:text-gray-300"
            }`}
          >
            {mode === "infinite" ? "계속" : `${mode}곡`}
          </button>
        ))}
      </div>
      {/* 메인 영역 */}
            ...
            !isRevealed ? (
              <div className="flex flex-col items-center gap-10 animate-in fade-in zoom-in duration-500 w-full">
                <div className="relative group">
                  <div className="w-48 h-48 rounded-full bg-gradient-to-br from-neutral-800 to-neutral-900 flex items-center justify-center text-7xl shadow-2xl ring-8 ring-indigo-500/10 transition-all">
                    🎵
                  </div>
                  <div className="absolute -bottom-2 -right-2 bg-indigo-600 rounded-full p-3 shadow-lg animate-bounce">
      ...
      {/* 우측 사이드바 */}
      {likedSongs.length > 0 && (
        <aside className="w-full md:w-72 bg-neutral-900/40 backdrop-blur-md rounded-3xl p-6 border border-neutral-800 sticky top-12 group transition-all shadow-2xl">
          ...
          <div className="max-h-[45vh] overflow-y-auto custom-scrollbar pr-1">
            <ul className="flex flex-col gap-3">
              {[...likedSongs].reverse().map((s, i) => (
                <li
                  key={i}
                  className="flex gap-3 items-center p-2 rounded-xl bg-neutral-800/30 animate-in slide-in-from-right-4"
                >
                  <img
                    src={s.imgUrl}
          ...
          <div className="mt-5 pt-4 border-t border-neutral-800 text-center">
            <button
              ...
              className="text-xs font-black text-indigo-500 hover:text-indigo-300 hover:underline transition-colors cursor-pointer"
            >
              여기를 클릭하면 결과보기 🏁
            </button>
          </div>
