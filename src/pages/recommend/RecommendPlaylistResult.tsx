...
function RecommendPlaylistResult() {
  ...
  const [hasResolvedResult, setHasResolvedResult] = useState<boolean>(false);
  ...
  const fetchedRequestKeyRef = useRef<string | null>(null);
  ...
  const fetchData = useCallback(async () => {
    const requestKey = `${trackName ?? ""}::${artistName ?? ""}`;
    if (!trackName || !artistName) {
      setData([]);
      setCheckedSongIds([]);
      setIsError(false);
      setIsLoading(false);
      setHasResolvedResult(true);
      return;
    }
    ...
    try {
      ...
      setData(songs);
      setCheckedSongIds(songs.map((item: RecommendedSong) => item.song.id));
  ...
  useEffect(() => {
    ...
    if (fetchedRequestKeyRef.current === requestKey) {
      return;
    }
    fetchData();
  }, [artistName, fetchData, trackName]);
  ...
  const handleSavePlaylist = async () => {
    ...
    try {
      ...
      if (axiosError.response?.status === 409) alert("같은 제목의 플레이리스트가 이미 존재합니다.");
    ...
    <div>
      ...
      {isLoading && (
        <div className="flex items-center justify-center py-48">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-lg" style={{ color: "var(--kf-sub)" }}>
              유사 곡 추천 중입니다...
            </p>
          </div>
        </div>
      ...
      {isError && !isLoading && (
        <div className="flex flex-col items-center justify-center gap-5 py-32">
          ...
          <p className="text-lg font-bold" style={{ color: "var(--kf-text)" }}>
            추천 중 문제가 발생했습니다.
          </p>
          <button
            onClick={fetchData}
      ...
      {!isLoading && !isError && data.length === 0 && trackName && artistName && (
        <div className="flex flex-col items-center justify-center gap-5 py-32">
          ...
          <p style={{ fontSize: 18, color: "var(--kf-sub)", textAlign: "center", lineHeight: 1.6 }}>
            <strong style={{ color: "var(--kf-text)", fontWeight: 800 }}>
              "{trackName} - {artistName}"
            </strong>
            <br />에 대한 추천 결과가 없습니다.
          </p>
      ...
      {/* 추천 플레이리스트 */}
      {hasResolvedResult && !isLoading && !isError && data.length > 0 && (
        <div className="flex flex-col items-center justify-center gap-10">
          {/* 커버 이미지 */}
          <div
            className="relative w-56 h-56 overflow-hidden transition-all cursor-pointer rounded-3xl"
            style={{
              ...
              <div className="flex flex-col items-center justify-center w-full h-full gap-2">
                ...
                {!savedPlaylistId && (
                  <span
                    ...
                    style={{ color: "var(--kf-brand, #6d5efc)" }}
                  >
                    커버 이미지 선택
                  </span>
                )}
          ...
          {/* 제목 · 설명 · 버튼 */}
          <div className="flex flex-col items-center w-full max-w-lg gap-2 px-6">
            {/* 제목 */}
            <div className="relative flex items-center justify-center w-full group">
              <textarea
                ...
                maxLength={100}
                className="w-full overflow-hidden text-3xl font-extrabold leading-tight text-center bg-transparent resize-none"
                style={{
                  color: "var(--kf-text)",
                  border:
            ...
            <div className="relative flex items-center justify-center w-full group">
              <textarea
                ...
                maxLength={200}
                className="w-full overflow-hidden text-sm text-center break-all bg-transparent resize-none"
                style={{
                  color: "var(--kf-sub)",
                  border:
            ...
            <div className="flex gap-3 mt-6">
              <button
                onClick={() => navigate(-1)}
                className="flex items-center gap-2 py-3 text-base font-semibold transition-all rounded-full px-7"
                style={{
              ...
              {!savedPlaylistId ? (
                <button
                  ...
                  disabled={isSaving}
                  className="flex items-center gap-2 py-3 text-base font-semibold text-white transition-all rounded-full px-7 disabled:opacity-50"
                  style={{
                    background: "linear-gradient(135deg, #6d5efc, #ff5ca8)",
                    boxShadow: "0 12px 28px rgba(109,94,252,0.28)",
                  }}
                >
                  <FaSave size={16} />
                ...
                <button
                  ...
                  className="flex items-center gap-2 py-3 text-base font-semibold text-white rounded-full px-7"
                  style={{
                    background: "linear-gradient(135deg, #6d5efc, #ff5ca8)",
                    boxShadow: "0 12px 28px rgba(109,94,252,0.28)",
                  }}
            ...
            <div className="kf-song-list" style={{ marginTop: 8 }}>
              {displayList.map((item) => (
                <div key={item.song.id} className="kf-rec-row">
                  <div className="kf-rec-row__main">
                    {!savedPlaylistId && (
                      <div
                        className="kf-rec-row__checkbox"
                        onClick={(e) => {
                          e.stopPropagation();
                  ...
                  {item.reason && (
                    <div
                      className="kf-rec-row__reason"
                      style={{ marginLeft: savedPlaylistId ? 0 : 52 }}
                    >
