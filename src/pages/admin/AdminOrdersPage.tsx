    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm mt-1" style={{color:"#677086"}}>총 {orders.length}건</p>
        </div>
      ...
      {orders.length === 0 ? (
        <p className="text-center py-16" style={{color:"#9199ad"}}>결제 내역이 없습니다.</p>
      ) : (
        <div className="flex flex-col gap-3">
          {orders.map((o) => {
            ...
            return (
              <div key={o.orderId} className="kf-admin-order-card">
                {/* 상단 */}
                <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
                  <div className="flex flex-wrap gap-x-5 gap-y-1 text-sm items-center">
                    <span className="text-xs" style={{color:"#9199ad"}}>#{o.orderId}</span>
                    <span className="font-semibold" style={{color:"#1f2430"}}>{itemSummary}</span>
                    <span style={{color:"#677086"}}>{o.receiverName} · {o.receiverPhone}</span>
                    <span className="text-xs" style={{color:"#9199ad"}}>{o.createdAt?.replace("T", " ").slice(0, 16)}</span>
                  </div>
                ...
                <div className="flex flex-wrap items-start gap-4 pt-3 kf-divider">
                  <div className="flex flex-col gap-1">
                    <p className="text-xs mb-1" style={{color:"#9199ad"}}>주문 상태</p>
                    <select
                      value={o.status}
                      onChange={(e) => handleStatusChange(o.orderId, e.target.value)}
                      className={`pl-3 pr-7 py-2 text-sm font-semibold cursor-pointer ${sl.color}`}
                    >
                  ...
                  <div className="flex-1 min-w-[260px]">
                    <p className="text-xs mb-1.5" style={{color:"#9199ad"}}>운송장</p>
                    {isEditing ? (
                      ...
                      <div className="flex items-center gap-3">
                        {o.trackingNumber ? (
                          <span className="text-sm" style={{color:"#677086"}}>{carrierName} · {o.trackingNumber}</span>
                        ) : (
                          <span className="text-sm" style={{color:"#9199ad"}}>미등록</span>
                        )}
                        <button
