  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-gray-400 text-sm mt-1">총 {users.length}명</p>
      </div>
        ...
        <table className="min-w-[860px] w-full text-sm">
          ...
          <tbody>
            {users.map((u) => {
              ...
              return (
                <tr key={u.id} className="border-b border-gray-800 hover:bg-white/[0.02] transition-colors whitespace-nowrap">
                  <td className="px-5 py-4 text-gray-600">{u.id}</td>
                  <td className="px-5 py-4 font-medium">{u.name}</td>
                  <td className="px-5 py-4 text-gray-300">{u.email || "-"}</td>
                  <td className="px-5 py-4 text-gray-300">{u.phoneNumber}</td>
                  <td className={`px-5 py-4 font-medium ${label.color}`}>{label.text}</td>
                  <td className="px-5 py-4">
                    <select
                      value={u.role}
                      onChange={(e) => handleRoleChange(u.id, e.target.value)}
                      className="bg-gray-800 border border-gray-700 rounded-lg pl-3 pr-6 py-1.5 text-sm text-white focus:outline-none focus:border-blue-500 cursor-pointer"
                    >
                      {ROLES.map((r) => (
                  ...
                  <td className="px-5 py-4 text-gray-500">{u.createdAt?.slice(0, 10)}</td>
                </tr>
              );
            })}
          </tbody>
