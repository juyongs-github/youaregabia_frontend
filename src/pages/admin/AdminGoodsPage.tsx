    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm mt-1" style={{color:"#4b5563"}}>총 {goodsList.length}개 상품</p>
        </div>
        <button
        ...
        <div className="kf-admin-order-card mb-6">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2">
              <label className="text-xs mb-1.5 block" style={{color:"#4b5563"}}>상품 설명</label>
            ...
            <div className="md:col-span-2">
              <label className="text-xs mb-1.5 block" style={{color:"#4b5563"}}>상품 이미지</label>
      ...
      <div className="kf-admin-table-wrap w-full overflow-x-auto">
              ...
              <tr key={g.goodsId} className="whitespace-nowrap">
                <td className="px-5 py-4">
                  <div className="w-10 h-10 rounded-lg flex items-center justify-center overflow-hidden" style={{background:"rgba(92,103,151,0.10)"}}>
                    {g.imageUrl ? <img src={`${api.defaults.baseURL ?? ""}${g.imageUrl}`} alt={g.name} className="w-full h-full object-cover" /> : <FaBox size={14} style={{color:"#64748b"}} />}
                  </div>
                </td>
                <td className="px-5 py-4 font-medium">{g.name}</td>
                <td className="px-5 py-4" style={{color:"#4b5563"}}>{categoryLabel[g.category] ?? g.category}</td>
        ...
        {goodsList.length === 0 && <p className="text-center py-12" style={{color:"#64748b"}}>등록된 상품이 없습니다.</p>}
      </div>
    </div>
  );
}
